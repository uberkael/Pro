...
def inicia_red():
	"""Inicia el servidor, o un cliente si no es posible
	Devuelve True si es servidor"""
	if(Network.is_port_in_use(Config.Network.puerto)):
		return False, Network.cliente()
	else:
		return True,  Network.servidor()


def objetivo_forzado_red(cliente):
	"Envia por el vector de predicciones de una imagen forzada"
	img, img_proces = prepara_imagen(Config.Network.archivo_forzado)
	prediccion = modelo.predict(np.array([img_proces]))
	cliente.envia_array(prediccion)
...
if __name__ == "__main__":
	...
	# procesa_objetivos()
	is_servidor, socket = inicia_red()
	if (socket.__class__ == Network.servidor):
		procesa_objetivos()
	else:
		objetivo_forzado_red(socket)
